...
def welcome(message):
    ...
    user_states[message.chat.id] = {'name': None, 'phone': None, 'tgId': None, 'card': 0, 'size': None, 'price': None}
...
def callback_message(callback):
    ...
    elif callback.data == 'need_more_size':
...
def end_of_work(message):
    if message.content_type == 'text':
        ...
        if re.match(r'^[0-9]+$', user_states[message.chat.id]['size']):
            user_states[message.chat.id]['price'] = float(user_states[message.chat.id]['product_data'][2].split(" ")[0]) * float(int(user_states[message.chat.id]['size'])/100)
            ...
            if float(user_states[message.chat.id]['size']) <= float(
                ...
                bottom2 = types.InlineKeyboardButton("Ввести размер повторно", callback_data='end_of_buy')
                bottom1 = types.InlineKeyboardButton("Закончить", callback_data='need_more_size')
                markup111.row(bottom1, bottom2)
                bot.send_message(message.chat.id,
                                 f"К сожалению, на нашем складе изначально нет такого количества товара, приносим свои извинения.\n"
                                 f"Нажмите на кнопку ввести размер повторно и введите размер меньший, чем {user_states[message.chat.id]['product_data'][3]}\n"
                                 f"Или нажмите на кнопку закончить, если Вам недостаточно товара, который есть в наличии в магазине",
                                 reply_markup=markup111)
